...
def make_cog_mosaics(
    ...
    if len(tiles_list) > 0:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            file_list_name = os.path.join(os.getcwd(), f"{product}_{time}_{freq}_{band}_{version}.txt")
            vrt_name = os.path.join(os.getcwd(), f"{product}_{time}--{freq}_{band}_{version}.vrt")
            output_name = os.path.join(os.getcwd(), f"{product}_mosaic_{time}_{freq}_{band}.tif")
