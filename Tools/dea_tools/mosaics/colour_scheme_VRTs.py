...
def _get_lc_colour_scheme(band,json_dir=None):
    """
    Get json file with colour scheme and return a dictionary ready to be used in code below

    Parameters:
    -----------
    band : str
        the LC level, either 'level3' or 'level4'
    json_dir : str
        path to folder containing json files with colour schemes. Defaults to None.
        If None, use the same directory of this python script
    """
...
def _create_vrt_landcover(
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for i in range(256):
            ...
            ET.SubElement(color_table, 'Entry', {
...
def _create_vrt_3bands_comp(
    ...
    """
    Create  three-bands composite visualisation

    Parameters:
    -----------
    product : str
        DEA product name (e.g., 'ga_ls8cls9c_gm_cyear_3').
    version : str
        Product version (e.g., '4-0-0').
    time : int or str
    ...
    freq : str
        The frequency of the summary product (e.g.,P1Y).
    cog_dir : str
        path to directory with continental COGs of the bands to use for the composites.
        E.g. 's3://dea-public-data/derivative/'
    output_dir : str
        local directory or s3 directory where to save ouptut. 
    r_channel_band : str
    ...
    if _is_s3(output_dir):
        ...
        out_is_s3 = False
        output_dir = os.path.join(output_dir, product, version, "continental_mosaics", f"{time}--{freq}")
        output_vrt = os.path.join(output_dir, f"{product}_mosaic_{time}--{freq}_{r_channel_band}-{g_channel_band}-{b_channel_band}.vrt")
        log.info(f"{run_id} - Output path: {output_vrt}")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Output paths for intermediate files
        temp_output_vrt = os.path.join(temp_dir, f"{product}_mosaic_{time}--{freq}_{r_channel_band}-{g_channel_band}-{b_channel_band}.vrt")
