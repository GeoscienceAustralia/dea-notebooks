...
def _get_training_data_for_shp(
    ...
    if zonal_stats is None:
        ...
        stacked = np.concatenate(([row[field]], flat_train.values))
...
def collect_training_data(
    ...
    if time_field is not None and time_delta is None:
        raise ValueError("If 'time_field' is provided 'time_delta' must also be provided.")
    
    if time_delta is not None:
        
        if time_field is None:
            raise ValueError("time_field must be supplied if time_delta is used")
    ...
    if zonal_stats:
        print("Applying zonal statistic: " + zonal_stats)
