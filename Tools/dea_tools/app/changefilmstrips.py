...
def run_filmstrip_app(
    ...
    if area > size_limit:
        ...
        # Obtain native CRS
        crs = mostcommon_crs(dc=dc, product="ga_ls5t_ard_3", query={"time": "1990", "geopolygon": geopolygon})
        ...
        if tide_range != (0.0, 1.0):
            ...
            print(f"    Keeping {len(ds.time)} observations with tides between {min_tide:.2f} and {max_tide:.2f} m")

        # Create time step ranges to generate filmstrips from
        bins_dt = pd.date_range(start=time_range[0], end=time_range[1], freq=pd.DateOffset(**time_step))
        ...
        time_steps = pd.cut(ds.time.values, bins_dt, labels=labels[:-1]).add_categories(labels[-1]).fillna(labels[-1])
        time_steps_var = xr.DataArray(time_steps, coords=[ds.time], name="timestep")

        # Resample data temporally into time steps, and compute geomedians
        geomedian_ds = ds.groupby(time_steps_var).apply(
            lambda ds_subset: xr_geomedian(ds_subset, num_threads=1, eps=0.2 * (1 / 10_000), nocheck=True)
        )
        ...
        output_array = geomedian_ds[["nbart_red", "nbart_green", "nbart_blue"]].to_array()
