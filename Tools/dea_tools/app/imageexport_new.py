...
sat_params = {
    'ga_ls_ard_3': {
        ...
        'styles': {
            ...
            'False colour': ('false_colour', ['nbart_swir_1', 'nbart_nir', 'nbart_green'])
        }
    },
    's2_ard_granule_nbar_t': {
        ...
        'styles': {
            ...
            'False colour': ('infrared_green', ['nbart_swir_2', 'nbart_nir_1', 'nbart_green'])
        }
    },
...
class imageexport_app(HBox):
    ...
    def __init__(self):
        ...
        instruction_text = "<p>Select the desired satellite data, imagery date and image style, zoom in until satellite imagery appears on the map, then draw a rectangle to select an area of imagery to export as a high-resolution image file.</p>"
        ...
        def update_geojson(target, action, geo_json):
            ...
            if area <= 50000:
                ...
                warning_text = '<span style="color: #ff5050"> <b>(Area to extract is too large, please select a smaller area)</b></span>'
        ...
        dropdown_styles = deawidgets.create_dropdown(self.styles_list,
                                                     self.styles_list[0])
        slider_abs = widgets.IntRangeSlider(value=[0, 2000],
                                            min=0,
                                            max=10000,
                                            step=25,
                                            description='',
        ...
        checkbox_pansharpen = deawidgets.create_checkbox(self.pansharpen,'Pansharpen Landsat')  
        ...
        # Add resolution dialog to class so it can be modified
    ...
    def run_app(self, change):
        ...
        with self.status_info:
            ...
            if self.rgb_array is None:
                ...
                self.query_params = {'time': (str(start_date), str(end_date)),
                                     'geopolygon': geopolygon}

                # Find matching datasets
                import itertools
                dss = [dc.find_datasets(product=i, **self.query_params) for 
                       i in sat_params[self.dealayer]['products']]
                ...
                can_pansharpen = self.style == 'True colour' and self.sensor in [  # and not self.resolution 
                    'Landsat-7', 'Landsat-8'
                ]
                ...
                if self.pansharpen and can_pansharpen:
                    ...
                    if self.resolution:
                        sat_params[self.dealayer]['resolution'] = (-self.resolution, self.resolution)
                ...
                if self.pansharpen and can_pansharpen:
                    ...
                    original_array = ds.drop('nbart_panchromatic').to_array().squeeze('time').values
                    rgb_array = match_histograms(image=rgb_array,
                                                 reference=original_array,
                                                 multichannel=True)
                
                if self.pansharpen and not can_pansharpen:
                    print('\nUnable to pansharpen Landsat 5 imagery, reverting to 30 m resolution')
                ...
                self.rgb_array = np.transpose(rgb_array, axes=[1, 2, 0])
        ...
        with self.status_info:
            ...
            print(f'\nExporting image. This may take several minutes...')
...
#                     if self.image_proc_funcs:
#                         for i, func in enumerate(self.image_proc_funcs):
