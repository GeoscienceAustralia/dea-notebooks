...
def WIT_drill(
    ...
    """
    Runs the Wetlands Insight Tool over a polygon. The code is based on the DEA Wetlands Insight Tool notebook.
    This function loads FC, WOs, and Landsat Data, and calculates Tasseled Cap Wetness, in order to summarise how the different classes have changed over time.
    
    The output is a pandas dataframe containing a timeseries of the normalised relative fractions of each class at each time-step. This forms the input to produce a stacked line plot.
    ...
    gdf : geopandas.GeoDataFrame
        The dataframe must only contain a single row containing the polygon you wish to interrogate
    time : tuple
        A tuple containing the time range over which to run the WIT. e.g. ("2014-01-01", "2015-01-01")
    ...
    dask_chunks : dict, optional
        ...
        the dimensions over which to chunk e.g. {'time':1, 'x':2048, 'y':2048}.
    ...
    bands = [
        f"nbart_{band}" for band in ("blue", "green", "red", "nir", "swir_1",
                                     "swir_2")
    ]
    ...
    # Divide Fractional Cover by 100 to keep them in [0,1]. Keeps data types the same in the output raster
    bs = ds_fc.bs / 100
    pv = ds_fc.pv / 100
    npv = ds_fc.npv / 100

    # Generate the WIT raster bands by creating an empty dataset called `output_rast` and populate with values from input datasets
...
def spatial_wit(ds, wetland_name):
    ...
    # Calculate the sum of the fractional cover values for pv, npv and bs then normalise the values for each class
    ...
    with imageio.get_writer(output_path, mode="I", duration=0.7,
                            loop=0) as writer:
