# The code in this file has been copied from the opendatacube-core repository
# located at: https://github.com/opendatacube/datacube-core
#
# Copyright (c) 2015-2025 ODC Contributors
# SPDX-License-Identifier: Apache-2.0
"""
This is a copy of the opendatacube-core masking module.

Two functions from odc.math are also included, dtype_is_float and valid_mask

FUNCTIONS:
----------
Data type and validation:
- dtype_is_float: Check if a numpy dtype is floating-point
- valid_mask: Create mask for valid (non-nodata) pixels in an array
- valid_data_mask: Returns boolean mask where data is not nodata (xarray)
- mask_invalid_data: Replace nodata values with NaN (xarray)

Flag inspection:
- list_flag_names: List available masking flag names for a variable
- describe_variable_flags: Get DataFrame or string describing all flags
- describe_flags_def: Format flags definition as string table
- get_flags_def: Extract flags_definition attribute from variable

Mask creation:
- make_mask: Create boolean mask from flag criteria (e.g., cloud_acca=False)
- create_mask_value: Convert flag definitions to bitmask values
- mask_to_dict: Convert bitmask value back to flag dictionary

Utilities:
- set_value_at_index: Set individual bit value in an integer bitmask
- generate_table: Format rows into pretty-printed table string

Internal helpers:
- _table_contents: Generate table rows from flags definition
- _order_bitdefs_by_bits: Sort bit definitions by bit number
"""
import os
