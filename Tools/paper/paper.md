...
* Generate Cloud-Optimised GeoTIFF (COG) mosaics and create VRTs with styling included for effortless visualisation in GIS software; this functionality is provided by the [`mosaic` sub-module](https://github.com/GeoscienceAustralia/dea-notebooks/blob/stable/Tools/dea_tools/mosaics/README.md), which accesses product tiles directly from S3 without requiring `datacube` or `STAC`.
