...
def load_agriculture_data():
    ...
    latitude = (-35.344, -35.342)
    longitude = (149.156, 149.158)
...
def run_agriculture_app(ds, dry_ndvi, irrigated_ndvi):
    """
    ...
    with fig_display:
        ...
        ax.set_ylim([-1, 1])
        ax.plot([today], [dry_ndvi], marker='o', color="#C6AB72", label='Dry NDVI')
        ax.plot([today], [irrigated_ndvi], marker='o', color="#7CFC00", label='Irrigated NDVI')
        ax.legend()
