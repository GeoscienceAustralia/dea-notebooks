...
Available functions:
    calculate indices  : NDVI, GNDVI, NDWI, NDMI, GI
...
def calculate_indices(ds, index):
    ...
    if index == 'NDWI-nir':
        ...
        print('The formula we are using is nir / green)')
        try:
            indexout = (ds.nir/ds.green)
        except AttributeError:
            try:
                # Assume the user wants to use nbart unless they explicity state otherwise
                indexout = (ds.nbart_nir_1/ds.nbart_green)
            except AttributeError:
                try:
                    indexout = (ds.nbar_nir_1 /ds.nbar_green)
                except:
                    print('Error! GI requires green and nir bands')
    ...
    try:
        ...
        print('Hmmmmm. I don\'t recognise that index. '
              'Options I currently have are NDVI, GNDVI, NDMI-green, NDMI-nir, NDWI and ModifiedNDWI., GI')
