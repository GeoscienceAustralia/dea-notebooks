...
def lc_colours(LC_colour_scheme = 'Level4', colour_bar = False):
    ...
    '''
    file = open("draft_lc_colour_definitions.txt", "r") #read file containing all colour scheme definitions
    ...
    LC_colour_scheme = LC_colour_scheme.lower() # make string all lower case just incase

    if LC_colour_scheme in valid_layer_list.keys():
        # create colour map from provided layer
        colours = valid_layer_list[LC_colour_scheme] # specify which sub-dictionary the colour scheme definitions will be taken from
        ...
        if colour_bar == False:
            ...
            return(cmap, norm, cblabels)#return specified level colour scheme

    else:
        print(f'layer name provided not a valid Land Cover layer. Please specifiy one of the following Land Cover layers {valid_layer_list.keys()}')
        return()
...
def lc_hex_convert(n:int, Land_cover_layer='Level4'):
    ...
    Land_cover_layer = Land_cover_layer.lower() # make string all lower case just incase
...
def lc_animation(ds, file_name='default_animation.gif', Land_cover_layer='level4', stacked_plot=False, animation_interval=500, width_pixels=500, dpi=400):
    ...
    layer_cmap, layer_norm, cblabels = lc_colours(Land_cover_layer, colour_bar = True)#create colour map and normalisation for specified lc layer
    ...
    def create_panda(da):
        ...
        list_classes = (np.unique(da, return_counts=False)).tolist() #list all class codes in dataset
        panda_table = pd.DataFrame(data=None, columns=list_classes) #create empty dataframe
        date_line = {} #empty dictionary to add lines to dataframe
        total_pix = int(np.sum(da.isel(time=1)))  # count all pixels, should be consistent
        
        for i in range(0,len(da.time)): #itterate through each year in dataset
            date = str(da.time[i].data)[0:10]
          
            for n in list_classes: #for each year itterate though each presant class number and count pixels
                number_of_pixles = int(np.sum(da.isel(time=i)  == n))
                percentage = (number_of_pixles / total_pix *100)
                date_line[n] = percentage
            panda_table.loc[date] = date_line #add each year's counts to dataframe
