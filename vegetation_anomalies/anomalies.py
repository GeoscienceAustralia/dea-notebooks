...
def load_ard(
    ...
    **kwargs,
    ...
    data_bands = [
        band for band in measurements if band not in (fmask_band, mask_contiguity)
    ]
    ...
    for product in products:
        ...
        if not ls7_slc_off and product == "ga_ls7e_ard_3":
            datasets = [
                i for i in datasets if i.time.begin < datetime.datetime(2003, 5, 31)
            ]
    ...
    if len(dataset_list) == 0:
        raise ValueError("No data available after filtering with " "predicate function")
    ...
    pq_mask = odc.algo.fmask_to_bool(ds[fmask_band], categories=fmask_categories)
