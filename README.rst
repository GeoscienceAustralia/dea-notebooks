...
* `develop branch <https://github.com/GeoscienceAustralia/dea-notebooks/tree/develop>`_: The ``develop`` branch is where notebooks are put that are almost ready to be shared publicly. Notebooks added to this branch will be pushed to the master branch <at some interval>. The ``develop`` branch is protected, and requires changes to be approved via a 'pull request' before they appear on the branch.
