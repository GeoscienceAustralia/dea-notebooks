...
int main(int argv, char** argc)
{
    ...
    string spfname, csvfname, tsbandnames, curdirc;
    time_t * tsbandtime;
	...
	hdrfname=curdirc+"/"+hdrfilestem;
	readhdrfile(hdrfname, ehd);
	tsbandnames=ehd.band_names;

	allbandnames=allbandnames+tsbandnames;

	if (i!=endyear)
	{
	    allbandnames=allbandnames+", ";
	}

    }

    cout<<"total tsbands = "<<tsbands<<endl;
    cout<<allbandnames<<endl;

    tsbandtime = new time_t[tsbands];
    getbandtime(allbandnames, tsbands, tsbandtime);
    ...
    pnum = irow*icol;
    maxmem=8.0*1000.0*1000.0*1000.0;
    blocksize=maxmem/(20*tsbands);
    rounds=pnum/blocksize+1;
    oss = tsbands*blocksize;
    ...
    bandavg= new double[oss];
    mndwi= new double[oss];
    noisemask= new char[oss];
    tsmask = new char[pnum*tsbands];


    pt=0;

    cout<<"Start noise detection processes ..... "<<endl;
    for(k=0;k<rounds;k++)
    {

	readonerounddata(bandnames, data, dirc, begyear, endyear, otbands, pt, tsbands, blocksize, pnum);

	calbandavg(data, bandavg, oss, otbands);
	calmndwi(data, mndwi, oss);
...
#pragma omp for schedule(dynamic) nowait
	...
	{
	    detectnoise_v2(bandavg, mndwi, i, blocksize, tsbands, noisemask, tsbandtime);
	}
...
#pragma omp for schedule(dynamic) nowait
	for(i=0;i<blocksize;i++)
	{
	    waterfilter(i, blocksize, tsbands,  bandavg, noisemask, mndwi);
	}
	...
	for(i=0;i<blocksize;i++)
	{
		...
		if (pt+i<pnum)
		{
		    tsmask[j*pnum+pt+i]=noisemask[j*blocksize+i];
		}
	    }
	}
	...
	cout<<"Round#"<<k+1<<" pcm/pcc = "<<pcm/(double)pcc<<endl;
	cout<<"Round#"<<k+1<<" of "<<rounds<<" completed."<<endl;
    }
	...
	cout<<"Before spatial filter, band#"<<j+1<<" pcm/pcc rate="<<pcm/(double)pcc<<", "<<(long)pcm<<", "<<(long)pcc<<endl;
    }
    ...
    obandnames.clear();
    sepbandnames(allbandnames, tsbands, obandnames);
    cout<<"Start applying spatial filter ..... "<<endl;
    spatialfilter_ap(tsbands,  irow, icol, tsmask, obandnames);
    spatialfilter_ap(tsbands,  irow, icol, tsmask, obandnames);
	...
	cout<<"after spatial filter, band#"<<j+1<<" pcm/pcc rate="<<pcm/(double)pcc<<", "<<(long)pcm<<", "<<(long)pcc<<endl;
    }
    



	ofname=dirc+"/tsmask.img";
	fout.open(ofname.c_str(), ios::binary);
	fout.write((char*) tsmask, pnum*tsbands*sizeof(char));
	fout.close();


	obandnames.clear();
	sepbandnames(allbandnames, tsbands, obandnames);
	ohdrfname=dirc+"/tsmask.hdr";
	writeenviheader(ohdrfname, "Cloud Mask, 0: clear, 1: invalid, 2: cloud shadows, 3: clouds", ehd.samples, ehd.lines, tsbands, 1, ehd.interleave, ehd.xstart, ehd.ystart, ehd.map_info,  ehd.wavelength_units, obandnames, ehd.projection_info, ehd.coordinate_system_string);
	...
	oss=curbands*irow*icol;
	oneyeartsmask=new char[oss];	

	pcm=0;
	pcc=0;
	for(j=0;j<oss;j++)
	{
	    oneyeartsmask[j]=tsmask[bandpt*pnum+j];
	    pidx=j%(irow*icol);
	    if (pidx<blocksize)
	    {
		if ((int)oneyeartsmask[j]==1)
		{
		    pcm++;
		}
		/*
	...
	cout<<"Year "<<i<<" tsmask invalid rate = "<<pcm/(double)pcc<<endl;
	...
	hdrfname=curdirc+"/NBAR_blue.hdr";
	readhdrfile(hdrfname, ehd);

	ofname=curdirc+"/tsmask.img";
	fout.open(ofname.c_str(), ios::binary);



	fout.write((char*) oneyeartsmask, oss*sizeof(char));
	fout.close();

	ohdrfname=curdirc+"/tsmask.hdr";
	obandnames.clear();
	sepbandnames(tsbandnames, curbands, obandnames);
	writeenviheader(ohdrfname, "Cloud Mask, 0: clear, 1: invalid, 2: cloud shadows, 3: clouds", ehd.samples, ehd.lines, curbands, 1, ehd.interleave, ehd.xstart, ehd.ystart, ehd.map_info,  ehd.wavelength_units, obandnames, ehd.projection_info, ehd.coordinate_system_string);

	for(j=0;j<otbands;j++)
	{
	    ohdrfname=curdirc+"/"+"NBAR_"+bandnames[j]+".hdr";
	    writeenviheader(ohdrfname, "Reflectance data", ehd.samples, ehd.lines, curbands, 2, ehd.interleave, ehd.xstart, ehd.ystart, ehd.map_info,  ehd.wavelength_units, obandnames, ehd.projection_info, ehd.coordinate_system_string);

	}

	delete [] oneyeartsmask;
    ...
    ohdrfname=dirc+"/urban_spec_5c.hdr";
    obandnames.clear();
    obandnames.push_back("Land cover class");
    
    writeenviheader(ohdrfname, "Land cover classes, 0: water, 1: vegetation, 3: urban", ehd.samples, ehd.lines, 1, 1, ehd.interleave, ehd.xstart, ehd.ystart, ehd.map_info,  ehd.wavelength_units, obandnames, ehd.projection_info, ehd.coordinate_system_string);
