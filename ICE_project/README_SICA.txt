...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
Readme for Summer Irrigated Cropping Area workflow (tailored for NMDB analysis, 20/05/2019)

    - An explanation of the output filenames are at the bottom on this readme
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
3. Run the 'build_vrt_and_mosaic.py' script to generate mosaics of the seasonal composites.
	...
	for things like image segmentation.
...
4. Run 'clip_ndvi_max.py' which clips the geotiffs from step 3 to an AOI, using
	...
	geotiffs into a single folder.  This script was initialy apart of the script in step 5
...
5. Run 'run_workaround_IE.py'. This does the bulk of the analysis by pulling in the 
	...
	the zonal stats over the segments, and filtering by area etc.  
	This script is run on megamem and is parallelized using the GNU Parallel library.
	It therefore requires you generate a text file of the input geotiff names which is then called
	in the qsub submission script.
	
		/g/data/r78/cb3058/dea-notebooks/ICE_project/workaround_IE_parallel_nmdb.py
		/g/data/r78/cb3058/dea-notebooks/ICE_project/run_workaround_IE_.sh

6. The results of step 5 produce a file called e.g. 'nmdb_Summer1991_92_80polys10ha.shp'
	These files contain only the polygons that achieved a threshold of >= 0.8 NDVI and are
	larger than 10ha in size.
	Using a rainfall anomaly timeseries from BOM for the AOI, open the anomalously dry and
	very-close-to-average years in ArcGIS and merge them to form a single file
	containg all the polygons from those years '<AOI>_80mask_allDryYears_merged.shp' (ArcGIS is
...
7. Using a good basemap, manually curate egregious commission errors contained in the 
	...
	and highlands grasslands in areas closer to the coast.

8. Run 'mask_results.py' which applies the curated mask from step 7 over the 'multithreshold.tif' output
	from step 5.  The results from this script have a suffix called '_IrrigatedMasked.shp'. These are your
	final results from the analysis.
	
	/g/data/r78/cb3058/dea-notebooks/ICE_project/mask_results.py
	
9.	Using the 'inspect_resuts.ipynb' notebook, generate plots showing the time-series of irrigated area.
...
<AOI>_<Season+Year>_sheperdSEG.kea
    This file conatins the results of the image segmentation algorithm provided with the RSGISlib library.
...
<AOI>_<Season+Year>_IrrigatedMasked.shp
